Hi 👋, I'm wanwan

💻 **Front-end Engineer**
- I'm currently learning **Html5,Css3,Scss,JavaScript(ES6+),TypeScript,Vue2,Vue3,Webpack,Vite**

[//]: # (- A developer who best understands customers and products, and the strongest engineer among those who communicate with customers and understand products.)

🚀 **What I do best**
- I design and implement high-performance, responsive front-end applications with a focus on user experience and smooth interface interactions. I am proficient in modern front-end frameworks such as Vue, and can build complex single-page applications (SPAs) while ensuring efficient page rendering and low latency. With deep expertise in CSS and JavaScript, I optimize webpage loading speeds and response times, ensuring consistency and compatibility across various devices and browsers.
- I have extensive experience in building real-time data interaction and streaming update features, and I am skilled at integrating with backend APIs, particularly when handling large-scale user interactions and data presentation. I am familiar with front-end automation build tools and containerized deployment, ensuring scalability and high availability of the system.
- I focus on system monitoring, log management, and performance optimization, using front-end performance analysis tools to detect and resolve potential performance bottlenecks, ensuring the best possible experience for end users.

📫 **How to Reach Me**
- Email: d2cc2ae@126.com

⚡ **Fun Fact**
- I like singing 🏸🏊‍♂️


[![GitHub Streak](https://streak-stats.demolab.com?user=wanwan&card_width=496)](https://git.io/streak-stats)


<h3 align="left">Languages and Tools:</h3>
<p align="left"> <a href="https://developer.mozilla.org/en-US/docs/Web/css3" target="_blank" rel="noreferrer"> <img src="https://skillicons.dev/icons?i=css" alt="css3" width="40" height="40"/> </a> <a href="https://developer.mozilla.org/en-US/docs/Web/html5" target="_blank" rel="noreferrer"> <img src="https://skillicons.dev/icons?i=html" alt="html5" width="40" height="40"/> </a> <a href="https://developer.mozilla.org/en-US/docs/Web/javascript" target="_blank" rel="noreferrer"> <img src="https://skillicons.dev/icons?i=js" alt="javascript" width="40" height="40"/> </a> <a href="https://developer.mozilla.org/en-US/docs/Web/typescript" target="_blank" rel="noreferrer"> <img src="https://skillicons.dev/icons?i=ts" alt="typescript" width="40" height="40"/> </a> <a href="https://developer.mozilla.org/en-US/docs/Web/vuejs" target="_blank" rel="noreferrer"> <img src="https://skillicons.dev/icons?i=vue" alt="vuejs" width="40" height="40"/> </a> <a href="https://developer.mozilla.org/en-US/docs/Web/webpack" target="_blank" rel="noreferrer"> <img src="https://skillicons.dev/icons?i=webpack" alt="webpack" width="40" height="40"/> </a></p>

